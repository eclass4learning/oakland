    ...
    function loginpage_hook() {
        ...
        if (!empty($authorizationcode)) {
                ...
                case 'linkedin':
                    $postreturnvalues = json_decode($postreturnvalues);
					      ...
					      ob_start();                    // start buffer capture
                                          var_dump( $postreturnvalues );           // dump the values
            ...
            if (!empty($accesstoken)) {
				error_log("I have access token: ".$useremail);
                ...
                if ($err = email_is_not_allowed($useremail)) {
                     ...
                     * Changes the message value being thrown here so I can identify this error specifically.
                     */
				    error_log("Rejected User email: ".$useremail);
                    throw new moodle_exception('INVALID_EMAIL_DOMAIN', 'auth_googleoauth2');
                    /**
                     * END OAKLAND CHANGES
                     */
                }
				error_log("Email validated: ".$useremail);
                ...
                if (empty($user)) {
					error_log("Creating user...: ".$useremail);
					...
					error_log("...Completed config: ".$useremail);
					...
					error_log("...Creating record: ".$useremail);
				...
				error_log("Authenticating: ".$useremail);
                ...
                if ($user) {
                    ...
                    // Redirection
                      ...
                      ob_start();                    // start buffer capture
   					  var_dump( $user );           // dump the values
   					  $contents = ob_get_contents(); // put the buffer into a variable
  					  ob_end_clean();                // end capture
   					//  error_log( $contents ); 
   					  
   					  error_log("Value of school: " . $user->profile['School']);
   					         
                    if (user_not_fully_set_up($user) or empty($user->profile['School']) ) {
                                                     
                        $urltogo = $CFG->wwwroot.'/user/edit.php';
                        // We don't delete $SESSION->wantsurl yet, so we get there later
                    } else if (isset($SESSION->wantsurl) and (strpos($SESSION->wantsurl, $CFG->wwwroot) === 0)) {
                        error_log("Sending to specific page..");
						$urltogo = $CFG->wwwroot.'/totara/dashboard/index.php?id=356';
                        unset($SESSION->wantsurl);
                    } else {
						error_log("Sending to dedault page..");
			     ...
			     error_log("No Access Token: ".$useremail);
