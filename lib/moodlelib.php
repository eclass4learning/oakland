...
function create_user_record($username, $password, $auth = 'manual') {
    ...
    $authplugin = get_auth_plugin($auth);
	error_log("...cur...starting process for: ".  $username);
    $customfields = $authplugin->get_custom_user_profile_fields();
    $newuser = new stdClass();
    if ($newinfo = $authplugin->get_userinfo($username)) {
		
		   ob_start();                    // start buffer capture
                                          var_dump( $newinfo );           // dump the values
                                          $contents = ob_get_contents(); // put the buffer into a variable
                                          ob_end_clean();                // end capture
                                          error_log( $contents );
		
		
		error_log("...cur...Gettinguser info ".  $username);
...
error_log("...cur...Looking at email ".  $username);
    if (!empty($newuser->email)) {
        if (email_is_not_allowed($newuser->email)) {
			error_log("...cur...rejecting email ".  $username . 'Email '.$newuser->email );
            unset($newuser->email);
        }
    }
	...
	error_log("...cur...Creating user ".  $username);
	...
	error_log("...cur...Saed profile ".  $username);
