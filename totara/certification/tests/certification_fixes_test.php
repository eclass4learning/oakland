...
class totara_certification_certification_fixes_testcase extends advanced_testcase {
    ...
    public function test_certif_fix_completion_copy_from_history() {
        ...
        foreach ($data->certifications as $certification_id) {
            foreach ($data->users as $user) {
                list($certif_completion, $prog_completion) = certif_load_completion($certification_id, $user->id);
        ...
        $target_user_id = reset($data->users)->id;
        $target_cert_id = reset($data->certifications);

        $before_certif_completions = $DB->get_records('certif_completion', [], 'id');
        $before_prog_completions = $DB->get_records('prog_completion', [], 'id');

        certif_fix_completions('fixrestorefromhistory', $target_cert_id, $target_user_id);
        ...
        foreach ($data->certifications as $certification_id) {
            foreach ($data->users as $user) {
                ...
                if ($certification_id == $target_cert_id && $user->id == $target_user_id) {
        ...
        $certif_completion_histories = $DB->get_records(
            ...
            ['certifid' => $target_cert_id, 'userid' => $target_user_id],
            'certifid, userid'
        );
