...
class totara_hierarchy_update_competencies_task_testcase extends advanced_testcase {
    ...
    public function test_competency_proficient_when_completion_data_is_deleted() {
        global $DB;
        ...
        $course1 = $this->create_self_completable_course([]);
        $user1 = $generator->create_user();
        ...
        $proficientid = $DB->get_field('comp_scale_values', 'id', ['proficient' => '1'], IGNORE_MULTIPLE);
        $non_proficient_ids = array_keys($DB->get_records('comp_scale_values', ['proficient' => '0'], 'id DESC', 'id'));
        $defaultid = reset($non_proficient_ids);

        /** @var competency_evidence_type $evidence */
        $evidence = competency_evidence_type::factory(['itemtype' => 'coursecompletion']);
        $evidence->iteminstance = $course1->id;
        $evidence->add($comp1);

        $this->make_user_mark_course_complete($user1, $course1);

        // Should not exist yet
        $this->assert_user_criteria_proficiency_measured($evidence, $user1, null);
        $this->assert_user_hold_competencies($user1, []);

        $this->run_task();
        // comp_criteria_record as well as comp_record should indicate that the user is proficient
        $this->assert_user_criteria_proficiency_measured($evidence, $user1, $proficientid);
        $this->assert_user_hold_competencies($user1, [$comp1->id]);

        // Now we clear all course completion data but don't touch comp_record
        $DB->execute('UPDATE {course_completions} SET timecompleted = NULL');
        $this->run_task();
        // comp_criteria_record should indicate proficiency not achieved, but comp_record should still indicate that the user is proficient
        $this->assert_user_criteria_proficiency_measured($evidence, $user1, $defaultid);
        $this->assert_user_hold_competencies($user1, [$comp1->id]);

        // Now we also update the comp_record manually - competency should not be reset to proficient
        $DB->execute('UPDATE {comp_record} SET proficiency = :notproficient', ['notproficient' => $defaultid]);
        $this->run_task();
        $this->assert_user_criteria_proficiency_measured($evidence, $user1, $defaultid);
        $this->assert_user_hold_competencies($user1, []);
    }
    ...
    /**
     * @param competency_evidence_type $evidence
     * @param stdClass $user
     * @param int|null $expected
     */
    private function assert_user_criteria_proficiency_measured(competency_evidence_type $evidence, stdClass $user, ?int $expected) {
        global $DB;

        $evidence_achievement = $DB->get_record('comp_criteria_record',
            [
                'itemid' => $evidence->id,
        ...
        if ($expected === null) {
            $this->assertFalse($evidence_achievement);
        } else {
            $this->assertEquals($expected, $evidence_achievement->proficiencymeasured);
        }
    }
