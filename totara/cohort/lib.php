...
function totara_cohort_notify_users($cohortid, $userids, $action, $delaymessages=false) {
    ...
    $cohort = $DB->get_record('cohort', array('id' => $cohortid), 'id, name, alertmembers, oaklandgroupid');
    ...
    foreach ($tousers as $touser) {
        ...
        // Oakland Modifications
        if (isset($cohort->oaklandgroupid)) {
            if (isset($requester_userid)) {
                $requester = $DB->get_record('user', array('id' => $requester_userid), 'firstname, lastname');
                if ($requester) {
                    $a->requester = (isset($requester->firstname) ? $requester->firstname . ' ' : '') . (isset($requester->lastname) ? $requester->lastname : '');
                }
            }
            $a->processlink = html_writer::link(new moodle_url('/oakland/groups/processrequests.php',array('id'=>$cohort->oaklandgroupid)),get_string('processrequest','oakland_groups'));
            $a->fullname = (isset($touser->firstname) ? $touser->firstname . ' ' : '') . (isset($touser->lastname) ? $touser->lastname : '');
            $emailsubject = $strmgr->get_string("msg:{$action}_{$towho}_emailsubject_oakland", 'totara_cohort', $a, $touser->lang);
            $notice = $strmgr->get_string("msg:{$action}_{$towho}_notice_oakland", 'totara_cohort', $a, $touser->lang);
        } else {
