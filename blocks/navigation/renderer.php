    ...
    protected function navigation_node($items, $attrs=array(), $expansionlimit=null, array $options = array(), $depth=1) {
        ...
        foreach ($items as $item) {
            ...
            if ($item->type == navigation_node::TYPE_ROOTNODE) {
                if ($content == "Courses" || $content == "My courses") {
                    foreach ($item->children as $categoriesToFilter) {
                        if ($categoriesToFilter->text == "Oakland Group Courses") {
                            $categoriesToFilter->remove();
                        }
                    }
                }
            } elseif ($item->type == navigation_node::TYPE_SETTING) {
                if ($content == "My learning") {
                    continue;
                }
            }
