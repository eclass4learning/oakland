...
class banner_upload_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('date_selector', 'timeavailabletill', get_string('enddate','block_banners'));


        $mform->addElement('text', 'urllink', get_string('urllink','block_banners'), array('size'=>50));
        $mform->setType('urllink',PARAM_RAW);
        ...
        $mform->setType('maxday',PARAM_INT);

        $mform->addElement('hidden', 'maxday', $this->_customdata['config']->height);
        $mform->setType('maxday',PARAM_INT);
    ...
    function validation($data, $files) {
        ...
        $errors = array();
        if($data['timeavailablefrom'] > $data['timeavailabletill']){
            $errors['timeavailabletill'] = get_string('timemismatch','block_banners');
        }
