...
$oaklandgroupname = trim(optional_param('oaklandgroupname', '', PARAM_NOTAGS));    // Group Names to search for
$numberofviews = trim(optional_param('numberofviews', '', PARAM_NOTAGS));      // Greater than number of views
$resourcetype = trim(optional_param('resourcetype', '', PARAM_NOTAGS));      // Resource Type
...
//Oakland Modifications
if (($oaklandgroupname == '' && $numberofviews == '' && $resourcetype == '' && !$search) || $showform) {
...
//Oakland Modifications
$oaklandsearchterms = array();
if ($oaklandgroupname != '') {
    $oaklandgrouprows = $DB->get_records_sql("SELECT * FROM {oakland_groups} WHERE lower(name) like lower(?)",array('%'.$oaklandgroupname.'%'));
    if ($oaklandgrouprows) {
        $groupids = '';
        $groupidseperator = '';
        foreach ($oaklandgrouprows as $oaklandgrouprow) {
            $groupids .= $groupidseperator.$oaklandgrouprow->id;
            $groupidseperator = ',';
        }
        $oaklandsearchterms['oaklandgroupid'] = '(p.oaklandgroupid in ('.$groupids.'))';
    } else {
        // force a not found
        $oaklandsearchterms['oaklandgroupid'] = '(p.oaklandgroupid = -999999)';
    }
}
if ($numberofviews != '') {
    $oaklandsearchterms['numberofviews'] = '(d.viewcount >= '.$numberofviews.')';
}
if ($resourcetype != '') {
    $oaklandsearchterms['resourcetype'] = "(p.resourcetype = '".$resourcetype."')";
}
if (!empty($oaklandsearchterms)) {
    $extra_or = '';
    $extrasql = 'AND (';
    foreach ($oaklandsearchterms as $oaklandsearchterm) {
        $extrasql .= $extra_or.$oaklandsearchterm;
        $extra_or = ' AND ';
    }
    $extrasql .= ')';
}
...
echo '<a href="search.php?id='.$course->id.
                         ...
                         '&amp;dateto='.$dateto.
                         '&amp;datefrom='.$datefrom.
                         '&amp;oaklandgroupname='.urlencode($oaklandgroupname).
...
function forum_print_big_search_form($course) {
    ...
    echo '<tr>';
    //Oakland Modifications
    if (strtolower($course->shortname) == 'classifieds') {
        echo '<td class="c0"><label for="subject">'.get_string('searchtopic', 'forum').'</label></td>';
    } else {
    ...
    //Oakland Modifications
    if (strtolower($course->shortname) == 'classifieds') {
        echo '<td class="c0"><label for="user">'.get_string('searchuserwhoposted', 'forum').'</label></td>';
    } else {
    ...
    if (strtolower($course->shortname) == 'classifieds') {
        echo '<tr>';
        echo '<td class="c0"><label for="oaklandgroupname">'.get_string('oaklandgroupname', 'forum').'</label></td>';
        echo '<td class="c1"><input type="text" size="35" name="oaklandgroupname" id="oaklandgroupname" value="'.s($oaklandgroupname, true).'" alt="" /></td>';
        echo '</tr>';
        echo '<tr>';
        echo '<td class="c0"><label for="numberofviews">'.get_string('numberofviews', 'forum').'</label></td>';
        echo '<td class="c1"><input type="text" size="10" name="numberofviews" id="numberofviews" value="'.s($numberofviews, true).'" alt="" /></td>';
        echo '</tr>';
        echo '<tr>';
        echo '<td class="c0"><label for="resourcetype">'.get_string('resourcetypesearch', 'forum').'</label></td>';
        echo '<td class="c1"><input type="text" size="50" name="resourcetype" id="resourcetype" value="'.s($resourcetype, true).'" alt="" /></td>';
        echo '</tr>';
    }
