...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if ($cm->name == 'Got It' || $cm->name == 'Want It') {
        $postuser->profilelink = new moodle_url('/user/view.php', array('id'=>$post->userid));
        if (isset($post->oaklandgroupid)) {
            $oaklandgrouprow = $DB->get_record_sql("select * from {oakland_groups} where id = ?",array($post->oaklandgroupid));
            if (isset($oaklandgrouprow)) {
                $postuser->groupname = $oaklandgrouprow->name;
                $postuser->grouplink = new moodle_url('/oakland/groups/apply.php', array('id'=>$post->oaklandgroupid));
            }
        }
    } else {
    ...
    //Oakland Modifications
    $namewithlink = get_string('bynameondate', 'forum', $by);
    if ($cm->name == 'Got It' || $cm->name == 'Want It') {
        if (isset($postuser->groupname)) {
            $by->groupname = html_writer::link($postuser->grouplink, $postuser->groupname);
            $namewithlink = get_string('bynamebygroupondate', 'forum', $by);
        }
    }
    $output .= html_writer::tag('div', $namewithlink, array('class'=>'author', 'role' => 'heading', 'aria-level' => '2'));
...
function forum_print_discussion_header(&$post, $forum, $group = -1, $datestring = "",
    ...
    //Oakland Modifications
    if ($forum->name == 'Want It' || $forum->name == 'Got It') {
        echo '<a href="'.$CFG->wwwroot.'/user/view.php?id='.$post->userid.'">'.$fullname.'</a>';
    } else {
    ...
    if ($forum->name == 'Want It' || $forum->name == 'Got It') {
        echo '<td class="replies">';
        echo '<a href="'.$CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'">';
        echo $post->viewcount.'</a>';
        echo "</td>\n";
        echo '<td class="replies">';
        echo $post->resourcetype;
        echo "</td>\n";
    }
    ...
    if ($forum->type !== 'qanda' || $canviewparticipants) {
        //Oakland Modifications
        if ($forum->name == 'Want It' || $forum->name == 'Got It') {
            echo '<a href="'.$CFG->wwwroot.'/user/view.php?id='.$post->usermodified.'">'.fullname($usermodified).'</a><br />';
        } else {
...
function forum_add_discussion($discussion, $mform=null, $unused=null, $userid=null) {
    ...
    $post->resourcetype = isset($discussion->resourcetype) ? '' : $discussion->resourcetype;
    if (isset($discussion->oaklandgroupid) && $discussion->oaklandgroupid != '0') {
        $post->oaklandgroupid = $discussion->oaklandgroupid;
    }
...
// If we want paging
    ...
    if ($displayformat == 'header') {
        ...
        // Oakland modifications
        if ($cm->name == 'Got It' || $cm->name == 'Want It') {
            echo '<th class="header topic" scope="col">'.get_string('topic', 'forum').'</th>';
        } else {
        ...
        // Oakland Modifications
        if ($forum->name == 'Want It' || $forum->name == 'Got It') {
            echo '<th class="header replies" scope="col">'.get_string('viewcount', 'forum').'</th>';
            echo '<th class="header replies" scope="col">'.get_string('resourcetype', 'forum').'</th>';
        }
