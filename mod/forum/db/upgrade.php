...
function xmldb_forum_upgrade($oldversion) {
 ...
 // Oakland modifications
    if ($oldversion < 2015061800) {
        ...
        // Forum savepoint reached.
        upgrade_mod_savepoint(true, 2015061800, 'forum');
    }
    ...
    // Oakland modifications
    if ($oldversion < 2015061804) {

        $table = new xmldb_table('forum_posts');
        $resourcetype = new xmldb_field('resourcetype', XMLDB_TYPE_CHAR, 50, null, false, null, 'None');

        if (!$dbman->field_exists($table, $resourcetype)) {
            $dbman->add_field($table, $resourcetype);
        }

        $table = new xmldb_table('forum_discussions');
        $viewcount = new xmldb_field('viewcount', XMLDB_TYPE_INTEGER, '10', null, false, null, 0);
